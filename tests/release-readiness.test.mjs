...
assert.equal(manifest.models.every(({ distributedWithSource }) => distributedWithSource === false), true);
...
assert.match(indexHtml, /源码版不包含 PP-MattingV2 权重/, '网页必须如实说明模型不随源码发布');
assert.match(readme, /License[\s\S]*待确定/, '用户未确认许可证前 README 必须标记待确定');
assert.match(privacy, /不会发送到像序服务器/, '隐私文档必须说明图片处理边界');

console.log('release readiness and optional-model tests passed');
